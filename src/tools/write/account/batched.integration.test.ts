...
describe("Batched write tools — live API (Base 8453)", { timeout: 30_000 }, () => {
  ...
  it("write.account.deleverage returns calldata", async () => {
    ...
    // Discovered account's collateral mix is dynamic, so the backend's
    // Tenderly simulation may fail. Either path exercises the tool: on
    // success we get calldata; on sim failure we must get the explicit
    // "simulation FAILED — do NOT broadcast" guard.
    if (result.isError) {
      expect(result.content[0].text).toContain("simulation FAILED");
      return;
    }
